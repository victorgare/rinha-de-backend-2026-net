...
namespace RinhaNet.Api.PreProcessor
{
    public static class VectorDatabaseBuilder
    {
        private const int Dimensions = 14;
        private static readonly string AssemblyPath = Path.GetDirectoryName(Assembly.GetExecutingAssembly()!.Location)!;
        private static readonly string DataPath = Path.Combine(AssemblyPath, "Resources", "references.json");
        private static readonly string OutputDir = "Data";
        ...
        public static async Task BuildAsync()
        {
            ...
            await foreach (var item in JsonSerializer.DeserializeAsyncEnumerable<VectorRecord>(jsonStream, options))
            {
                if (item is null)
                    continue;
